...
class UNet(nn.Module):
    def __init__(self,img_channel,channels=[64,128,256,512,1024],time_embedding_size=256) -> None:
        super().__init__()
        ...
        for i in range(len(channels)-1):
            self.encoder_convs.append(ConvBlock(channels[i],channels[i+1],time_embedding_size))
        ...
        for i in range(len(channels)-2):
            self.decoder_convs.append(ConvBlock(channels[-i-1],channels[-i-2],time_embedding_size))
    ...
    def forward(self,x,t):
        ...
        for i,conv in enumerate(self.deconvs):
            ...
            x = self.decoder_convs[i](x,t_embedding)
...
if __name__ == '__main__':
    ...
    batch_pred_noise_t = unet(batch_x_t, batch_t)
