...
class ConvBlock(nn.Module):
    def __init__(self,in_channel,out_channel,time_embedding_size) -> None:
        super().__init__()
    ...
    def forward(self,x,time_embedding): #time_embedding:(batch_size,time_embedding_size)
        x = self.seq1(x)
        time_embedding = self.relu(self.time_embedding_linear(time_embedding)).view(x.size(0),x.size(1),1,1)
        return self.seq2(x+time_embedding) 